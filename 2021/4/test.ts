...
`;
  ...
  const [order, boards] = parseInput(new Input(input));
  const [sum, called] = solvePart1(order, boards);
  assertEquals(called, 24);
  assertEquals(sum, 188);
});
...
`;
  ...
  const [order, boards] = parseInput(new Input(input));
  const [sum, called] = solvePart2(order, boards);
  assertEquals(called, 13);
  assertEquals(sum, 148);
});
